...
function App() {
    ...
    const [showMatchesTitle, setShowMatchesTitle] = useState(false);
    ...
    const fetchBrandMatches = () => {
        if (selectedLeagueId) { // Only fetch if a league is selected
            ...
            axios.get(requestUrl)
                .then(response => {
                    const matchesWithBrandData = response.data.data.map(match => {
                        // Assuming the response includes brand data
                        match.brand = match.brand || {};
                        return match;
                    });
                    setShowMatchesTitle(true);
    ...
    const clearSelection = () => {
        ...
        setShowMatchesTitle(false);

    }

    return (
        <Container className="App">
            <h1 className="text-center my-4">Stats Perfom</h1>
                ...
                <Dropdown onSelect={(key, event) => {
                    ...
                    <Dropdown.Menu>
                        {leagues.map((league, index) => (
                            <Dropdown.Item key={index} eventKey={league.name}
                                           id={league.id}>{league.name}</Dropdown.Item>
                        ))}
                    </Dropdown.Menu>
                </Dropdown>
                <Dropdown onSelect={(key, event) => {
            ...
            <div>
                {showMatchesTitle && <h2>Upcoming Matches</h2>}
